...
public final class advancepayment extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        jPanel10.add(delete, new org.netbeans.lib.awtextra.AbsoluteConstraints(340, 200, 180, 50));
   ...
   // else
        ...
        if (y > x) {
            try {
                String sql = "SELECT MIN(remaining) as remaining FROM eslam.advanceamount where serial='" + serial + "' AND date between '" + curr + "' AND '" + end + "'";
                ...
                if (rs.next()) {
                    if (rs.getString("remaining") != null && rs.getString("remaining").trim().length() > 0) {
                        ...
                        String sql111 = "INSERT INTO eslam.advanceamount (name,serial,amount,remaining,date) VALUES "
                        ...
                        String sqll = "SELECT salary FROM eslam.employee where serial='" + serial + "'";
                        pst = (PreparedStatement) con.prepareStatement(sqll);
                        rs = pst.executeQuery();
                        if (rs.next()) {
                            int remianing = Integer.parseInt(rs.getString("salary"));
                            int remain = remianing - x;
                            String sql1 = "INSERT INTO eslam.advanceamount (name,serial,amount,remaining,date) VALUES "
    ...
    public void tablelord() {
        try {
            String sqll = "SELECT serial,name,amount,remaining,date FROM eslam.advanceamount  ORDER BY serial ASC,\n"
                    + "    remaining DESC;";
            pst = (PreparedStatement) con.prepareStatement(sqll);
            rs = pst.executeQuery();
    ...
    public void comboxfromcombo() {
        ...
        cmserial.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent event) {
                ...
                try {
                    String sql = "SELECT name  FROM eslam.employee where serial='" + cmserial.getSelectedItem().toString() + "'";
                    pst = (PreparedStatement) con.prepareStatement(sql);
                    rs = pst.executeQuery();
                    if (rs.next()) {
                ...
                try {
                    String sql = "SELECT MIN(remaining)as remaining  FROM eslam.advanceamount where serial='" + cmserial.getSelectedItem().toString() + "' and date between '" + curr + "' AND '" + end + "'";
                    pst = (PreparedStatement) con.prepareStatement(sql);
                    rs = pst.executeQuery();
                    if (rs.next()) {
                        if (rs.getString("remaining") != null && rs.getString("remaining").trim().length() > 0) {
                            ...
                            String sql9 = "SELECT salary  FROM eslam.employee where serial='" + cmserial.getSelectedItem().toString() + "'";
    ...
    public void serial() {
        try {
            String sql = "SELECT serial  FROM eslam.employee order by CONVERT(replace(serial,'EM',''),UNSIGNED INTEGER) ASC ";
            pst = (PreparedStatement) con.prepareStatement(sql);
            rs = pst.executeQuery();
            while (rs.next()) {
    ...
    public void name() {
        try {
            String sql = "SELECT name  FROM eslam.employee ";
            pst = (PreparedStatement) con.prepareStatement(sql);
            rs = pst.executeQuery();
            while (rs.next()) {
